...
if __name__ == "__main__":
    ...
    video_path      = 0
    video_save_path= ""
    # video_path = "E:\dataforre_fa\Obama_v.mp4"
    # video_save_path = "E:\dataforre_fa\obama.mp4"
    video_fps       = 25.0
    #-------------------------------------------------------------------------#
    ...
    if mode == "predict":
        ...
        while True:
            ...
            if image is None:
                ...
                picname = "图片:"
                picname = picname.encode("gbk").decode(errors="ignore")
                cv2.imshow(picname,r_image)
                cv2.waitKey(0)
        ...
        if video_save_path!="":
            # *'XVID'该参数是MPEG-4编码类型，文件名后缀为.avi，可指定结果视频的大小
            fourcc = cv2.VideoWriter_fourcc(*'XVID')
            # 使用get()获取视频帧数(opencv3以上版本) frame_count = capture.get(cv2.CAP_PROP_FRAME_COUNT)
            # 使用get()获取视频高度和宽度(opencv3以上版本)
