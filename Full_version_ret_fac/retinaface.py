...
def cv2ImgAddText(img, label, left, top, textColor=(255, 255, 255)):
    ...
    # 解释：有点类似于函数声明一样，告诉你就在这个图上绘图
    draw = ImageDraw.Draw(img)
    label = label.encode('utf-8')
    # draw.text((位置),'要写进去的文字',fill=,font=font)
...
class Retinaface(object):
    ...
    def encode_face_dataset(self, image_paths, names):
        ...
        for index, path in enumerate(tqdm(image_paths)):
            ...
            # 看输入的图片的尺寸是不是640*640,如果不是那么需要reshape一下
            ...
            with torch.no_grad():
                # -----------------------------------------------------------#
                #   图片预处理，归一化。这里需要把深度调整到前面,然后最前面加上batch的维度
                # -----------------------------------------------------------#
    ...
    def detect_image(self, image):
        ...
        for face_encoding in face_encodings:
            ...
            # -----------------------------------------------------#
            # 调用face_distance算出来的欧氏距离和阈值比较,返回欧式距离和一个是否同一人脸的bool
