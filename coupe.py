...
output_folder_entrainement = './Data_set/images_sorties/entrainement'  # Dossier pour enregistrer les images rognées pour entraînement
output_folder_validation = './Data_set/images_sorties/validation'  # Dossier pour enregistrer les images rognées pour validation
output_log_path_entrainement = './Data_set/text_infos/logs_output_entrainement.csv'
output_log_path_validation = './Data_set/text_infos/logs_output_validation.csv'
...
if len(image_files) == 0:
    ...
    for idx, filename in enumerate(image_files):
        ...
        # Créer un nom de fichier pour l'image rognée（包括后缀名）
        new_filename = f"{original_name}_cropped.jpg"

        if idx <(2/3*len(image_files)):
            ...
            # Ajouter une ligne au journal pour l'image rognée（不记录文件后缀）
            # La première ligne : image originale -> image rognée
            ...
            # Si ce n'est pas la première image, utiliser la précédente image rognée
...
# Enregistrer les données dans un fichier CSV（不记录后缀名）
